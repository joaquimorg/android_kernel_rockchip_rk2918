...
#define SENSOR_NAME fcam
#define SENSOR_V4L2_IDENT V4L2_IDENT_FCAM
...
{
    if(pinfo->change_cam_orientation) {
        pinfo->change_cam_orientation(g_i2c_client,  mode);
    }
}
EXPORT(change_cam_orientation);
#endif
...
static struct cam_sensor_info * all_cam_info[]= {
    ...
    &cam_sensor_info_nt99250,
    &cam_sensor_info_gc0307,
    &cam_sensor_info_ov7675,
    &cam_sensor_info_gc0308,
    &cam_sensor_info_gt2005
};
			       ...
			       struct i2c_client *client)
{
    ...
    pinfo=0;
    SENSOR_DG("No cam found in front\n", __func__);
    return -ENODEV;
}
...
static ssize_t fname_show(struct device *dev, struct device_attribute *attr, char *_buf)
{
    if(pinfo) {
			 ...
			 const struct i2c_device_id *did)
{
	...
	device_create_file(fcam_dev, &dev_attr_name);
